...
def create_team(team_data: TeamCreate, db: Session = Depends(get_db)):
    if team_data.hltv_id and team_service.get_team_by_hltv(db, team_data.hltv_id):
        raise HTTPException(status_code=409, detail="Team with this HLTV ID already exists")
    return team_service.create_team(db, team_data)


@router.get("/", response_model=list[TeamResponse])
def get_teams(skip: int = 0, limit: int = 50, db: Session = Depends(get_db)):
